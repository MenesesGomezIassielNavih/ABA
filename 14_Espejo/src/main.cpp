...
//shader para el espejo
Shader shader1;
//shader para el espejo
Shader shaderSkybox_1;
...
GLuint skyboxTextureID, textureID, textureID_;
...
std::vector<std::string> faces1 = { "../Textures/espejo/right.jpg",
		"../Textures/espejo/left.jpg",
		"../Textures/espejo/top.jpg",
		"../Textures/espejo/bottom.jpg",
		"../Textures/espejo/front.jpg",
		"../Textures/espejo/back.jpg" };
...
std::vector<std::string> fileNames = { "../Textures/mp_bloodvalley/blood-valley_ft.tga",
...
glm::mat4 modelMatrixCubo1 = glm::mat4(1.0f);
...
std::map<std::string, glm::vec3> blendingUnsorted = {
		...
		{"fountain", glm::vec3(0.0, 3.0, -30.0)},
		{"fire", glm::vec3(0.0, 0.0, 7.0)}
	//	{"fire2", glm::vec3(0.0, 20.0, -30.0)}
...
GLuint feedback[2];
GLuint drawBuf = 1;
float particleSize = 5.0, particleLifetime = 6.0;
...
void initParticleBuffers() {
	...
	for (GLuint i = 0; i < nParticles; i++) {
		//recoge la direccion de la velocidad
		...
		//Escalar la configuracion de la magnitud de la velocidad 0.6f, 0.8f
		velocity = glm::mix(0.6f, 0.7f, ((float)rand() / RAND_MAX));
	...
	glBindBuffer(GL_ARRAY_BUFFER, initVel);  
	glBufferSubData(GL_ARRAY_BUFFER, 0, size, data); //lBufferSubData(GL_ARRAY_BUFFER, 0, size, data);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shader1.initialize("../Shaders/6.2.cubemaps.vs", "../Shaders/6.2.cubemaps.fs");
	shaderSkybox_1.initialize("../Shaders/6.2.skybox.vs", "../Shaders/6.2.skybox.fs");
	...
	mayowModelAnimate.loadModel("../models/luffy/todas_luffy_6.fbx");
	...
	{
		camera1->setPosition(glm::vec3(0.0, 0.0, 3.0));	
	}
...
//***********************************************************
    ...
    for (int i = 0; i < ARRAY_SIZE_IN_ELEMENTS(types1); i++) {
		cubemapTexture = Texture(faces1[i]);
		cubemapTexture.loadImage(true);
		if (cubemapTexture.getData()) {
			glTexImage2D(types1[i], 0, GL_RGB, cubemapTexture.getWidth(), cubemapTexture.getHeight(), 0,
			GL_RGB, GL_UNSIGNED_BYTE, cubemapTexture.getData());
		} else
			std::cout << "Failed to load texture" << std::endl;
...
//********************************************************
	...
	generator.seed(rd());
	int size = nParticles * 2;  //int size = nParticlesFire * 2;  
	...
	shaderParticlesFire.setInt("ParticleTex", 0);  //0
	shaderParticlesFire.setInt("RandomTex", 1);
	shaderParticlesFire.setFloat("ParticleLifetime", particleLifetime);
	shaderParticlesFire.setFloat("ParticleSize", particleSize);
	shaderParticlesFire.setVectorFloat3("Accel", glm::value_ptr(glm::vec3(0.0f,-0.3f,0.0f)));
...
bool processInput(bool continueApplication) {
	...
	if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS){
		...
		modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0, 0, 0.5));
		animationLuffyIndex = correr;
	}else if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS){
		modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0, 0, -0.5));
		animationLuffyIndex = correr;
...
void applicationLoop() {
	...
	modelMatrixFountain = glm::translate(modelMatrixFountain, glm::vec3(0.0, 0.0, -30.0));
	modelMatrixFountain[3][1] = terrain.getHeightTerrain(modelMatrixFountain[3][0] , modelMatrixFountain[3][2]) + 0.2;
	modelMatrixFountain = glm::scale(modelMatrixFountain, glm::vec3(8.0f, 8.0f, 8.0f));

	modelMatrixCubo1 = glm::translate(modelMatrixCubo1, glm::vec3(0.0f,0.0f, 0.0f));
	...
	shaderSkybox_1.turnOn();
	shaderSkybox_1.setInt("skybox", 0);
...
void renderScene(bool renderParticles){
	...
	boxEspejo.setScale(glm::vec3(10.0, 10.0, 10.0));
	boxEspejo.setPosition(glm::vec3(-10.0, 0.0, 0.0));
	boxEspejo.setOrientation(glm::vec3(0.0, 0.0, 0.0));
	boxEspejo.render();
	glEnable(GL_CULL_FACE);*/
	...
	//model[3][1] = terrain.getHeightTerrain(model[3][0] , model[3][2]) + 0.2;
	model = glm::translate(model, glm::vec3(0.0f, 5.0f, 0.0f));
	model = glm::scale(model, glm::vec3(10.0f, 10.0f, 10.0f));

	
	shader1.setMatrix4("model",1, false, glm::value_ptr(model));
	shader1.setMatrix4("view",1, false, glm::value_ptr(view));
	shader1.setMatrix4("projection",1, false, glm::value_ptr(projection));
	
	if(!isThirdCamera)
		shader1.setVectorFloat3("cameraPos", glm::value_ptr(camera1->getPosition()));
	else	
		shader1.setVectorFloat3("cameraPos", glm::value_ptr(camera->getPosition()));
	...
	modelMatrixMayowBody = glm::scale(modelMatrixMayowBody, glm::vec3(0.010, 0.010, 0.010));
	mayowModelAnimate.setAnimationIndex(animationLuffyIndex);
	...
	for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
		if(it->second.first.compare("aircraft") == 0){
			...
			modelMatrixParticlesFountain[3][1] = terrain.getHeightTerrain(modelMatrixParticlesFountain[3][0], modelMatrixParticlesFountain[3][2]) + 0.36 * 8.0;
	...
	//******************SKYBOX por medio de un CUBO primitivo
	// dibujando el skybox al final
	glDepthFunc(GL_LEQUAL);  // cambiando la funcion depth para que la peueba de profundidad pase,
							// cuando sea igual al contenido del buffer de profundidad
	shaderSkybox_1.turnOn();
	if(!isThirdCamera)
		view = glm::mat4(glm::mat3(camera1->getViewMatrix())); 
	else 
		view = glm::mat4(glm::mat3(camera->getViewMatrix())); // remueve la traslación de la matrix vista

	shaderSkybox_1.setMatrix4("view",1, false, glm::value_ptr(view));
	shaderSkybox_1.setMatrix4("projection",1, false, glm::value_ptr(projection));
	// skybox cube
	glBindVertexArray(skyboxVAO);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, textureID);
	glDrawArrays(GL_TRIANGLES, 0, 36);
	glBindVertexArray(0);
	glDepthFunc(GL_LESS); //  pone la funcion depth devuelta a su estado por default
}
